...
const PhaserRainEffect: React.FC = () => {
  useEffect(() => {
    ...
    function create(this: Phaser.Scene) {
      ...
      // 그래픽에 색상과 크기를 설정
      graphics.fillStyle(0xffffff, 1); // 파란색 채우기
      graphics.fillRect(0, 0, 1, 30); // 폭 2, 높이 10의 직사각형 모양

      // 그래픽을 텍스처로 변환하여 사용
      const textureKey = "rain";
      graphics.generateTexture(textureKey, 1, 30); // 'rain'이라는 텍스처 키로 생성

      // 파티클 시스템 생성
      this.add.particles(0, 0, textureKey, {
        x: { min: 0, max: window.innerWidth }, // 파티클의 X축 범위
        y: 0, // 파티클의 시작 Y 좌표
        lifespan: 3000, // 파티클의 수명 (밀리초)
        speedY: { min: 200, max: 400 }, // Y축 속도 범위
        scale: { start: 0.7, end: 0.2 }, // 파티클 크기 변화
